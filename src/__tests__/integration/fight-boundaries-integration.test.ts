...
describe("Integration — fight boundary parity and visual alignment", () => {
  ...
  it("renders both charts and asserts first boundary positions align within 2 pixels (approx)", () => {
    ...
    const { container: dealtContainer } = render(
      <DamageDealtChart series={dealtSeries} fightBoundaries={dealtSeries.fightBoundaries} />,
    );
    ...
    const dealtLines = dealtContainer.querySelectorAll('[data-role="ref-line"]');
    const takenLines = takenContainer.querySelectorAll('[data-role="ref-line"]');
    ...
    const dealtTimestamps = dealtSeries.points.map((p) => p.timestamp.getTime());
