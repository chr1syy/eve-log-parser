...
export async function POST(request: NextRequest) {
  try {
    ...
    const { userId, log } = body as { userId?: string; log?: { sessionId?: string } }

    if (!userId || !log?.sessionId) {
      return NextResponse.json({ error: 'Missing userId or log.sessionId' }, { status: 400 })
    }
  ...
  try {
    ...
    for (const file of files) {
      ...
      try {
        ...
        const { entries: _e, ...meta } = parsed as { entries: unknown; [key: string]: unknown }
        metas.push(meta)
      } catch {
        // Skip corrupt files
      }
