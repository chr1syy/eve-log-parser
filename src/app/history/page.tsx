...
export default function HistoryPage() {
  const router = useRouter();
  const { isAuthenticated, isLoading: authLoading } = useAuth();
  const { logs, selectLog, deleteLog, loadUserLogs } = useLogsContext();
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedLogId, setSelectedLogId] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    fetchLogs();
  }, [isAuthenticated, authLoading, loadUserLogs]);
  ...
  // Convert logs to metadata
  const logsMetadata: LogMetadata[] = logs.map((log) => ({
    id: log.sessionId,
    filename: log.fileName || "combat_log.txt",
    uploadedAt: log.parsedAt || new Date(),
        ...
        <Panel>
          {loading ? (
            ...
            <div className="space-y-3">
              ...
              {/* Log rows */}
              <div className="space-y-2 max-h-[calc(100vh-300px)] overflow-y-auto">
                {logsMetadata.map((logMeta) => (
                  <LogHistoryItem
                    key={logMeta.id}
                    id={logMeta.id}
                    filename={logMeta.filename}
                    uploadedAt={logMeta.uploadedAt}
                    fileSize={logMeta.fileSize}
                    combatDuration={logMeta.combatDuration}
                    isSelected={selectedLogId === logMeta.id}
                    onView={handleViewLog}
                    onDelete={handleDeleteLog}
                  />
                ))}
              ...
              <div className="mt-6 pt-4 border-t border-border text-sm text-text-muted">
                ...
                <p className="mt-1">
                  ...
                  <span className="text-cyan-glow font-mono">
                    {(() => {
                      const totalMinutes = logsMetadata.reduce(
                        (sum, log) => sum + (log.combatDuration || 0),
                        0,
                      );
                      if (totalMinutes < 60)
